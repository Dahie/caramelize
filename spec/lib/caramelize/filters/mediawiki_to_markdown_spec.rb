...
describe Caramelize::MediawikiToMarkdown do # rubocop:todo RSpec/FilePath, RSpec/SpecFilePathFormat
  ...
  describe '#run' do
    ...
    context 'when bold and italic' do
      let(:body) { "'''''Text is italic'''''" }
    ...
    context 'when preformatted section' do
      context 'with one space' do
        let(:body) { " line 1\n line 2" }

        it { is_expected.to eq "    line 1\n    line 2" }
      end
    end
    ...
    context 'when wikilink' do
      ...
      context 'with spaces' do
        let(:body) { '[[Lemma Lemma]]' }

        it { is_expected.to eq '[[Lemma Lemma]]' }
      end
